...
navButtons.forEach((button) => {
  button.addEventListener("click", () => {
      ...
      // Khi vào trang lịch sử, áp dụng bộ lọc đã lưu (nếu có)
...
saveResultButton.addEventListener("click", () => {
  if (currentResult) {
    const confidencePct = currentResult.confidence === null || currentResult.confidence === undefined ? 0 : Math.round((currentResult.confidence || 0) * 100);
    const drugGroup = currentResult.drug_group || currentResult.drug_group_name || (currentResult.top_predictions && currentResult.top_predictions[0] && (currentResult.top_predictions[0].drug_group || currentResult.top_predictions[0].disease_vi || currentResult.top_predictions[0].disease)) || "";
    const status = currentResult.error ? 'failure' : 'success';
    savedResults.push({
    ...
    updateHistoryEmptyState();
    renderRecentActivity();
    showToast('Lưu lịch sử thành công', false);
  } else {
...
// ========================= HISTORY FILTER & EXPORT (Enhancements) =========================
...
function showToast(message, isError = false, timeout = 3000) {
  ...
  t.style.color = 'var(--primary-contrast)';
  t.style.opacity = '1';
  clearTimeout(t._to);
  t._to = setTimeout(() => { t.style.opacity = '0'; }, timeout);
}

// Utility: parse savedAt (vi-VN locale) into Date
function parseSavedDate(entry) {
  if (entry.savedAtISO) return new Date(entry.savedAtISO);
  const s = entry.savedAt || '';
  // Attempt dd/mm/yyyy
...
// Load/save filter state
const HISTORY_FILTER_KEY = 'pharma_history_filter_v1';
function loadHistoryFilter() {
  try { return JSON.parse(localStorage.getItem(HISTORY_FILTER_KEY) || '{}'); } catch { return {}; }
}
function saveHistoryFilter(obj) { try { localStorage.setItem(HISTORY_FILTER_KEY, JSON.stringify(obj)); } catch {} }

// Default groups (can be extended)
const HISTORY_DRUG_GROUPS = [
  'kháng sinh','tim mạch','giảm đau','hô hấp','tiêu hóa','nội tiết','vitamin và khoáng chất','thuốc kháng virus','thuốc giảm đau hạ sốt'
];

// Initialize filter modal controls
(function initHistoryFilterModule(){
  const btnFilter = document.getElementById('btn-history-filter');
  const modal = document.getElementById('modal-history-filter');
  const modalClose = document.getElementById('modal-history-filter-close');
  ...
  const groupList = document.getElementById('history-group-list');
  const groupSearch = document.getElementById('history-group-search');
  const confidenceInput = document.getElementById('history-confidence');
  const confidenceVal = document.getElementById('history-confidence-val');
  const statusChecks = Array.from(document.querySelectorAll('.history-status'));
  const sortSelect = document.getElementById('history-sort');
  const applyBtn = document.getElementById('history-filter-apply');
  ...
  // render groups
  function renderGroupChips(filterText=''){
    groupList.innerHTML = '';
    const kw = (filterText||'').toLowerCase();
    HISTORY_DRUG_GROUPS.filter(g => !kw || g.toLowerCase().includes(kw)).forEach(g => {
      const btn = document.createElement('button');
      btn.type = 'button'; btn.className = 'text-button'; btn.style.padding='6px 10px'; btn.style.borderRadius='16px';
      btn.dataset.group = g; btn.textContent = g;
  ...
  groupSearch?.addEventListener('input', (e) => renderGroupChips(e.target.value));

  btnFilter.addEventListener('click', () => {
    const saved = loadHistoryFilter();
    // populate modal with saved values
    rangeSelect.value = saved.range || 'any';
    if (saved.start_date) startDateInput.value = saved.start_date; else startDateInput.value = '';
    if (saved.end_date) endDateInput.value = saved.end_date; else endDateInput.value = '';
    confidenceInput.value = saved.confidence_min || 0;
    confidenceVal.textContent = (saved.confidence_min || 0) + '%';
    sortSelect.value = saved.sort || 'newest';
    // status
    statusChecks.forEach(ch => ch.checked = (saved.status || []).includes(ch.value));
    // groups
    const selectedGroups = new Set(saved.groups || []);
    Array.from(groupList.children).forEach(btn => {
      btn.classList.toggle('is-active', selectedGroups.has(btn.dataset.group));
    });

    // show custom date inputs only when custom selected
    startDateInput.style.display = rangeSelect.value === 'custom' ? 'inline-block' : 'none';
    endDateInput.style.display = rangeSelect.value === 'custom' ? 'inline-block' : 'none';
  ...
  rangeSelect?.addEventListener('change', () => {
    ...
    startDateInput.style.display = v === 'custom' ? 'inline-block' : 'none';
    endDateInput.style.display = v === 'custom' ? 'inline-block' : 'none';
  });

  confidenceInput?.addEventListener('input', () => { confidenceVal.textContent = confidenceInput.value + '%'; });

  function collectFilterFromUi(){
    const groups = Array.from(groupList.children).filter(b=>b.classList.contains('is-active')).map(b=>b.dataset.group);
    ...
    const obj = {
      ...
      groups: groups,
      confidence_min: Number(confidenceInput.value || 0),
      status: statuses,
  ...
  applyBtn?.addEventListener('click', () => {
    const f = collectFilterFromUi();
    saveHistoryFilter(f);
    modal.classList.add('is-hidden');
    applyHistoryFiltersAndRender();
    showToast('Áp dụng bộ lọc', false);
  });
  ...
  clearBtn?.addEventListener('click', () => {
    ...
    modal.classList.add('is-hidden');
    applyHistoryFiltersAndRender();
    showToast('Đã xóa bộ lọc', false);
  });

  resetBtn?.addEventListener('click', () => {
    const def = { range:'any', groups:[], confidence_min:0, status:[], sort:'newest' };
    saveHistoryFilter(def);
    // reflect immediately
    rangeSelect.value = def.range;
    startDateInput.value = '';
    endDateInput.value = '';
    confidenceInput.value = def.confidence_min;
    confidenceVal.textContent = def.confidence_min + '%';
    statusChecks.forEach(ch=>ch.checked=false);
    Array.from(groupList.children).forEach(btn=>btn.classList.remove('is-active'));
    sortSelect.value = def.sort;
    showToast('Đã khôi phục mặc định', false);
  });
  ...
  exportMenu?.querySelectorAll('button[data-export]').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        await exportFilteredHistory(fmt);
      } catch (err) {
        console.error(err);
...
// Apply filters on savedResults and re-render history
async function applyHistoryFiltersAndRender() {
  const f = loadHistoryFilter();
  let list = Array.isArray(savedResults) ? savedResults.slice() : [];

  // date range
  const now = new Date();
  function inRange(d) {
    if (!d) return false;
    if (f.range === 'any' || !f.range) return true;
    if (f.range === 'today') {
      const today = new Date(); return d.toDateString() === today.toDateString();
    }
    if (f.range === '7days') {
      const cutoff = new Date(now.getFullYear(), now.getMonth(), now.getDate()-6); return d >= cutoff;
    }
    if (f.range === '30days') {
      const cutoff = new Date(now.getFullYear(), now.getMonth(), now.getDate()-29); return d >= cutoff;
    }
    if (f.range === 'month') {
  ...
  list = list.filter(entry => {
    const d = parseSavedDate(entry);
    if (d && !inRange(d)) return false;
    if (f.groups && f.groups.length > 0) {
      const g = (entry.drug_group || '').toLowerCase();
      if (!f.groups.some(gr => g.includes(gr.toLowerCase()))) return false;
    }
    if (typeof f.confidence_min === 'number' && f.confidence_min > 0) {
      const c = Number(entry.confidence || 0);
      if (c < f.confidence_min) return false;
    }
    if (f.status && f.status.length > 0) {
      const s = entry.status || 'success';
  ...
  list.sort((a,b) => {
    if (sort === 'newest') return (new Date(b.savedAtISO||b.savedAt) - new Date(a.savedAtISO||a.savedAt));
    if (sort === 'oldest') return (new Date(a.savedAtISO||a.savedAt) - new Date(b.savedAtISO||b.savedAt));
    if (sort === 'confidence_desc') return (Number(b.confidence||0) - Number(a.confidence||0));
    if (sort === 'confidence_asc') return (Number(a.confidence||0) - Number(b.confidence||0));
    if (sort === 'group_az') return String((a.drug_group||'').toLowerCase()).localeCompare(String((b.drug_group||'').toLowerCase()));
    return 0;
  });

  // render
  document.querySelectorAll('.user-history-card').forEach((card) => card.remove());
  list.slice().reverse().forEach((entry) => historyList.prepend(createHistoryCard(entry)));
  updateHistoryEmptyState();

  // update stats
  const statsEl = document.getElementById('history-filter-stats');
  if (statsEl) statsEl.textContent = `Tổng bản ghi: ${list.length}`;
}

// Export filtered history
async function exportFilteredHistory(format = 'csv') {
  const f = loadHistoryFilter();
  // reuse filtering logic by calling applyHistoryFiltersAndRender but fetch results from savedResults
  let list = Array.isArray(savedResults) ? savedResults.slice() : [];
  // apply same filtering logic inline (to avoid re-render side effects)
  const now = new Date();
  function inRangeEntry(d) {
    if (!d) return false;
    if (f.range === 'any' || !f.range) return true;
    if (f.range === 'today') {
      const today = new Date(); return d.toDateString() === today.toDateString();
  ...
  list = list.filter(entry => {
    const d = parseSavedDate(entry);
    if (d && !inRangeEntry(d)) return false;
    if (f.groups && f.groups.length > 0) {
      const g = (entry.drug_group || '').toLowerCase();
      if (!f.groups.some(gr => g.includes(gr.toLowerCase()))) return false;
    }
  ...
  if (!list || list.length === 0) { showToast('Không có dữ liệu để xuất', true); return; }

  if (format === 'print') {
    const w = window.open('', '_blank');
    w.document.write('<html><head><title>Lịch sử xuất</title></head><body>');
    list.forEach(item => { w.document.write(`<div style="margin-bottom:12px;"><strong>${item.disease}</strong> — ${item.savedAt} — ${item.confidence}% — ${item.drug_group || ''}<div>${(item.symptoms||[]).join(', ')}</div><div>${item.notes||''}</div></div>`); });
    w.document.write('</body></html>');
    w.document.close();
    w.print();
    showToast('Đã mở cửa sổ in', false);
    return;
  }

  // CSV
  const rows = [ ['disease','savedAt','confidence','drug_group','symptoms','notes'] ];
  list.forEach(r => rows.push([r.disease, r.savedAt, r.confidence || '', r.drug_group || '', (r.symptoms||[]).join('; '), (r.notes||'').replace(/\n/g,' ')]));
  const csv = rows.map(r => r.map(c => '"'+String(c||'').replace(/"/g,'""')+'"').join(',')).join('\n');
  ...
  if (format === 'csv') {
    ...
    showToast('Xuất CSV thành công', false);
    return;
  }
  ...
  if (format === 'xlsx') {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'history_export.xlsx'; a.click(); URL.revokeObjectURL(url);
    showToast('Xuất Excel thành công', false);
    return;
  }
}

// Apply filters on initial load (restore saved filter)
(function(){
  try { const f = loadHistoryFilter(); if (f && Object.keys(f).length>0) applyHistoryFiltersAndRender(); } catch(e){}
})();